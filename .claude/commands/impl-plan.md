...
Named `/impl-plan` (not `/plan`) to stay distinct from Claude Code's built-in Plan Mode (`EnterPlanMode`) and from this project's other ECC planning commands (`/plan-prd`, `/plan-canvas`, `/prp-plan`).

Run inline by default. Do not call the Task tool or any subagent by default. This keeps `/impl-plan` usable from plugin installs that ship commands without agent files.
...
```
User: /impl-plan I need to add real-time notifications when markets resolve
